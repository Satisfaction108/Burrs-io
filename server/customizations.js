// Customization definitions for nametags and spikes

export const NAMETAG_CUSTOMIZATIONS = [
  {
    id: 'nametag_default',
    name: 'Default',
...
export const SPIKE_CUSTOMIZATIONS = [
  {
    id: 'spike_default',
    name: 'Default',
...
export function getCustomizationById(id) {
  const nametag = NAMETAG_CUSTOMIZATIONS.find(c => c.id === id);
  if (nametag) return { ...nametag, type: 'nametag' };
  
  const spike = SPIKE_CUSTOMIZATIONS.find(c => c.id === id);
  if (spike) return { ...spike, type: 'spike' };
